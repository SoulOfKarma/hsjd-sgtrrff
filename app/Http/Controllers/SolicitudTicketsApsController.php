...
class SolicitudTicketsApsController extends Controller
{
    ...
    public function getTicketsKPI(){
        try {
            ...
            WHEN solicitud_tickets_aps.id_estado = 5 || solicitud_tickets_aps.id_estado = 6 THEN ROUND(((SELECT COUNT(id_estado) FROM solicitud_tickets_aps WHERE id_estado BETWEEN 5 AND 9)*100)/(SELECT COUNT(id_estado) FROM solicitud_tickets),1)
            END) AS porcentaje"))
            ->join('estado_solicituds','solicitud_tickets_aps.id_estado','=','estado_solicituds.id')
